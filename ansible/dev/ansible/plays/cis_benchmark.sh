...
# Running CIS-BENCHMARK for CentOS 5
run_ansible_centos5() {
    if [ "$OS_VERSION" == "5" ]; then
        ansible-playbook -v -i $HOSTNAME, -e 'ansible_python_interpreter=/usr/bin/python2.6' --ask-become-pass cis_benchmark.yml
    fi
}
...
ansible-playbook -v -i $HOSTNAME, --ask-become-pass cis_benchmark.yml    
